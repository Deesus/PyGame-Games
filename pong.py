...
def new_game():
    ...
    paddle1_pos = [[0, HEIGHT/2 - PAD_HEIGHT/2], [PAD_WIDTH, HEIGHT/2 - PAD_HEIGHT/2], [PAD_WIDTH, PAD_HEIGHT/2 + HEIGHT/2], [0, PAD_HEIGHT/2 + HEIGHT/2]]
    paddle2_pos = [[WIDTH - PAD_WIDTH, HEIGHT/2 - PAD_HEIGHT/2], [WIDTH, HEIGHT/2 - PAD_HEIGHT/2], [WIDTH, PAD_HEIGHT/2 + HEIGHT/2], [WIDTH - PAD_WIDTH, PAD_HEIGHT/2 + HEIGHT/2]]
...
def draw(canvas):
    ...
    if (paddle1_pos[0][1] + paddle1_vel >= 0) and (paddle1_pos[3][1] + paddle1_vel <= HEIGHT):
        for coordinate in paddle1_pos: coordinate[1] += paddle1_vel

    # right
    if (paddle2_pos[0][1] + paddle2_vel >= 0) and (paddle2_pos[3][1] + paddle2_vel <= HEIGHT):
        for coordinate in paddle2_pos: coordinate[1] += paddle2_vel

    # draw paddles 
    canvas.draw_polygon(paddle1_pos, 1, "red", "red")   #polygon lines are drawn in clockwise direction
    ...
    if ball_pos[0] + (BALL_RADIUS + 1) >= WIDTH - PAD_WIDTH - 1:
        if paddle2_pos[0][1] - (BALL_RADIUS+1) <= ball_pos[1] <= paddle2_pos[3][1] + (BALL_RADIUS+1):
            bounce()
        else:
            if score1 < 98: score1 += 1
            spawn_ball()
    #left side:
    if ball_pos[0] - (BALL_RADIUS + 1) <= PAD_WIDTH:
        if paddle1_pos[0][1] - (BALL_RADIUS + 1) <= ball_pos[1] <= paddle1_pos[3][1] + (BALL_RADIUS+1):
            bounce()
        else:
            if score2 < 98: score2 += 1
            spawn_ball()
